<?php session_start();
		...
		if($row["chucvu"]=='admin'){
			echo "Xin chào admin ".$row["hoten"].".<a href='xu_li/dangxuat.php'>Đăng xuất</a>";
		}
		else {
						...
						<?php 
							...
							if (isset($_SESSION['id']) && $_SESSION['id']){
								if($op=='tai_khoan'){
									if($action=='sua'){
										$id = $_GET['id'];
										$result = $conn->query("SELECT * FROM tai_khoan where id = '$id'");
										$row = $result->fetch_assoc();
										...
										echo "<form action='xu_li/them.php?op=tai_khoan' method='POST'><table width='500px' style='border-spacing:8px'>"; 
									...
									if($action=='sua'){
										...
										$mo_ta=$row["mo_ta"];
										echo "<form action='xu_li/sua.php?op=mon_an&ten=$ten_mahoa' enctype='multipart/form-data' method='POST'><table width='500px' style='border-spacing:8px'>"; 
										...
										echo "<form action='xu_li/them.php?op=mon_an' enctype='multipart/form-data' method='POST'><table width='500px' style='border-spacing:8px'>"; 
	        					...
	        					echo 'Bạn chưa <a href=dangnhap.php>đăng nhập</a>';
	     					}
