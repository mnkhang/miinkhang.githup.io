        ...
        <div class="center_right">
            ...
            <div class="mon_an">
                ...
                if ($trang>0){
                    ...
                    $_GET['nlieu']=$_GET['o2'];
                    $_GET['sapxep']=$_GET['o3'];
                    $_GET['tu_khoa']=$_GET['o4'];
                }
                if (isset($_GET['tu_khoa'])) {
                    if ($_GET['loai']=='all' && $_GET['nlieu']=='all' ){
                        if($_GET['sapxep']==tang){
                            $result = $conn->query("SELECT * FROM thuc_an where lower(ten) like '%".$_GET[tu_khoa]."%' order by luot_xem");
                        }
                        else $result = $conn->query("SELECT * FROM thuc_an where lower(ten) like '%".$_GET[tu_khoa]."%' order by luot_xem DESC");
                    }
                    else if($_GET['nlieu']=='all' || $_GET['loai']=='all'){
                        if($_GET['sapxep']==tang){
                            $result = $conn->query("SELECT * FROM thuc_an WHERE (lower(ten) like '%".$_GET[tu_khoa]."%') and (loai='".$_GET[loai]."' or nguyen_lieu = '".$_GET[nlieu]."') order by luot_xem");
                        }
                        else $result = $conn->query("SELECT * FROM thuc_an WHERE (lower(ten) like '%".$_GET[tu_khoa]."%') and (loai='".$_GET[loai]."' or nguyen_lieu = '".$_GET[nlieu]."') order by luot_xem DESC");
                    }
                    else {
                        if($_GET['sapxep']==tang){
                            $result = $conn->query("SELECT * FROM thuc_an WHERE (lower(ten) like '%".$_GET[tu_khoa]."%') and loai='".$_GET[loai]."' and nguyen_lieu = '".$_GET[nlieu]."' order by luot_xem");
                        }
                        else $result = $conn->query("SELECT * FROM thuc_an WHERE (lower(ten) like '%".$_GET[tu_khoa]."%') and loai='".$_GET[loai]."' and nguyen_lieu = '".$_GET[nlieu]."' order by luot_xem DESC");
                    }
                }
                ...
                $i=-1;$u=0;
                        ...
                        while ($row = $result->fetch_assoc()){
                            $i++;
                        if($i<($trang-1)*10||$u>9){ //hiển thị 5 món với số trang tương ứng.
                            continue;
                        }
                        if ($u==5) {
                            echo "<tr>";
                        ...
                        if($result2->num_rows==0){
                            ...
                            echo "<h5>".$result3->num_rows." người thích</h5>";
                        }
                        else {
                    ...
                    for($i=1; $i<=$sotrang; $i++){
                        ...
                        if($trang==$i){
                            echo "<a href='de_xuat.php?trang=$i&o1=$_GET[loai]&o2=$_GET[nlieu]&o3=$_GET[sapxep]&o4=$_GET[tu_khoa]' style='color:red'>Trang $i</a>&emsp;";
                        }
                        else echo "<a href='de_xuat.php?trang=$i&o1=$_GET[loai]&o2=$_GET[nlieu]&o3=$_GET[sapxep]&o4=$_GET[tu_khoa]'>Trang $i</a>&emsp;";
                    }echo "</center>";
                    ?>
                </div>
            </div>
